...
public class GameTree {
    ...
    public int findValueHelper(TreeNode root, Player player) {
        if(root == null) {
        ...
        if(player == Player.ONE) {
            for(int i = 0; i < root.numChildren; ++i) {
                ...
                int childValue = findValueHelper(child, other(player));
                root.value = Math.max(childValue, root.value);
            }
        } else {
            ...
            for(int i = 0; i < root.numChildren; ++i) {
                TreeNode child = root.children[i];
                int childValue = findValueHelper(child, other(player));
                root.value = Math.min(childValue, root.value);
            }
        }
        return root.value;
    }
    ...
    int findValue() {
        ...
        return findValueHelper(root, player);
    }
